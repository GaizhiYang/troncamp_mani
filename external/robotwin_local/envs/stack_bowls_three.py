...
class stack_bowls_three(Base_Task):
    ...
    def load_actors(self):
        ...
        import os as _odb
        if _odb.environ.get("DUMP_BOWLS"):
            print("[BOWLS]", [list(np.round(np.asarray(p.p[:2]), 3)) for p in bowl_pose_lst], flush=True)
    ...
    def play_once(self):
        bowls = [self.bowl1, self.bowl2, self.bowl3]
        base_xy = np.asarray(self.bowl1_target_pose, dtype=float)[:2]
        # bowl1 -> fixed base center; bowls 2/3 -> nest on the bowl below
        prev_arm = self.move_bowl_geo(bowls[0], [float(base_xy[0]), float(base_xy[1]), _GEO_BASE_Z])
        for i in range(1, 3):
            if not self.plan_success:
                continue
            nb = np.asarray(bowls[i].get_pose().p, dtype=float)
            next_arm = ArmTag("left" if nb[0] < 0 else "right")
            # retract the previous arm clear of the stack on arm-switch (selective; always-retract regressed)
            if prev_arm is not None and next_arm != prev_arm:
                self.move(self.back_to_origin(arm_tag=prev_arm))
                if not self.plan_success:
                    self.plan_success = True
            below = np.asarray(bowls[i - 1].get_pose().p, dtype=float)
            prev_arm = self.move_bowl_geo(bowls[i], [float(below[0]), float(below[1]), float(below[2]) + _GEO_NEST_OFF])
    ...
    def check_success(self):
        ...
        eps2 = 0.04
        import os as _og
        if _og.environ.get("DUMP_GRADED"):
            base = 0.74 + self.table_z_bias
            s = 0
            if np.all(abs(bowl1_pose[:2] - bowl2_pose[:2]) < eps2) and bowl2_pose[2] > base + 0.015:
                s = 1
                if np.all(abs(bowl2_pose[:2] - bowl3_pose[:2]) < eps2) and bowl3_pose[2] > base + 0.045:
                    s = 2
            print(f"[GRADED] stacks={s} score={s/2.0}", flush=True)
